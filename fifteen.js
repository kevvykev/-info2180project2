...
window.onload = function ()
{
	...
	{
	    div[i].style.backgroundImage=("background.jpg");
		...
		div[i].onmouseover = function()
		{
			...
			{
				this.style.border = "2px solid DarkTurquoise";
	...
	shufflebutton.onclick = function()
	{
		...
		for (var i=0; i<250; i++)
		{
			...
			if (rand == 0)
			{
				var tmp = calcUp(horizontalSpace, verticalSpace);
				if ( tmp != -1)
				{
					swap(tmp);
				}
			...
			if (rand == 1)
			{
				var tmp = calcDown(horizontalSpace, verticalSpace);
				if ( tmp != -1) 
				{
					swap(tmp);
				}
			...
			if (rand == 2)
			{
				var tmp = calcLeft(horizontalSpace, verticalSpace);
				if ( tmp != -1)
				{
					swap(tmp);
				}
			...
			if (rand == 3)
			{
				var tmp = calcRight(horizontalSpace, verticalSpace);
				if (tmp != -1)
				{
					swap(tmp);
				}
...
{
	if (calcLeft(horizontalSpace, verticalSpace) == (pos-1))
	{
		return true;
	}

	if (calcDown(horizontalSpace, verticalSpace) == (pos-1))
	{
		return true;
	}

	if (calcUp(horizontalSpace, verticalSpace) == (pos-1))
	{
		return true;
	}

	if (calcRight(horizontalSpace, verticalSpace) == (pos-1))
	{
		return true;
	}
}
function Blink()
{
	...
	if (blink == 0)
	{
		...
		body[0].style.backgroundColor = "#00BFFF";
		alert('you win');
		return;
	}
	if (blink % 2)
	{
		var body = document.getElementsByTagName('body');
		body[0].style.backgroundColor = "#48D1CC";	
	}
	else
	{
		var body = document.getElementsByTagName('body');
...
function swap (pos) {
	...
	div[pos].style.top = verticalSpace;
	verticalSpace = temp;

	temp = div[pos].style.left;
	div[pos].style.left = horizontalSpace;
	horizontalSpace = temp;
}
